...
class PersistedScoutsUserManager(UserManager):
...
class PersistedScoutsUser(User):

    objects = PersistedScoutsUserManager()
    ...
    scouts_leader_groups: List[AbstractScoutsGroup] = []
    ...
    @property
    def scouts_leader_group_names(self):
        return [group.group_admin_id for group in self.scouts_leader_groups]
    ...
    def has_role_leader(self, group: ScoutsGroup) -> bool:
        for function in self.persisted_scouts_functions.filter(end__isnull=True):
            if function.is_leader_function(scouts_group=group):
                return True
        return False
    ...
    def has_role_group_leader(self, group: ScoutsGroup) -> bool:
        ...
        """
        for function in self.persisted_scouts_functions.filter(end__isnull=True):
            if function.is_group_leader_function(scouts_group=group):
                return True
